...
class RedBlackTree():
    ...
    def rotate_right_left(self, my_node):  # правое вращение к левой части дерева
        child = my_node.right
    ...
    def rotate_left_right(self, my_node): # левое вращение к правой части дерева
        child = my_node.left
    ...
    def draw_tree(self,node, offset=0):
        if node is not None:
            self.draw_tree(node.right, offset + 15)
            print(' ' * offset + str(node.data) + ' (' + node.color + ')')
            self.draw_tree(node.left, offset + 15)
